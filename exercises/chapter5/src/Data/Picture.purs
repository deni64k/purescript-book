...
showShape (Clipped pic c w h) =
  "Clipped [location: " <> showPoint c <> ", width: " <> show w <> ", height: " <> show h <> ", picture: " <> show pic <> "]"
...
shapeBounds (Clipped pic c w h) = intersect clippingBounds $ foldl union emptyBounds bounds
  where
    clippingBounds = shapeBounds $ Rectangle c w h
    bounds = shapeBounds <$> pic
